...
df_1[[3]] = rnorm(n_1, mean = -2.5)
df_1[[4]] = rbinom(n_1, 6, 0.95)
df_1[[5]] = rexp(n_1, 1.6)
df_1[[6]] = rpois(n_1, 2.8)
df_1[[7]] = rgamma(n_1, 6.5, .5)
df_1[[8]] = 1
...
df_2[[3]] = rnorm(n_2, mean = -2.7)
df_2[[4]] = rbinom(n_2, 6, .99)
df_2[[5]] = rexp(n_2, 1.8)
df_2[[6]] = rpois(n_2, 3.4)
df_2[[7]] = rgamma(n_2, 6.75, .5)
df_2[[8]] = 1
...
write.csv(synth,"test3.csv", row.names=FALSE, quote = FALSE)


ggplot(synth)+
  geom_density(aes(x=...7, fill = as.factor(...8)), alpha=.5, bins = 10)
